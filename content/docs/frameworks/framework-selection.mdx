...
- 应用平台：Dify 这类画布 + RAG + Agent 控制台，和代码框架分开评估。
- Google 生态：Google ADK（Python / TS / Go / Java）与 Vertex / Gemini 部署。
...
flowchart TD
  ...
  B -->|是| D[优先 LangGraph、Mastra workflow 或 Google ADK]
  B -->|否| E{需要多角色协作?}
  ...
  I -->|否| K{要拖拽应用平台和知识库?}
  K -->|是| L[Dify]
  K -->|否| M[先用模型 SDK + 自己的状态机]
  D --> N{绑定 Gemini / Vertex?}
  N -->|是| O[Google ADK]
  N -->|否| P[LangGraph 或 Mastra]
```
...
| Google ADK | Python 为主，另有 TS / Go / Java | Gemini / Vertex 部署、Workflow + HITL + 评测 UI | 和 Google 云绑定度要评估，钉版本测 resume |
| Dify | 应用平台，可自托管 | 画布工作流、RAG、Agent、BaaS API | 许可证有额外条件；不替代精细状态机 |
