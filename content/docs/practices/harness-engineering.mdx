...
## 开源对照：DeepSeek Harness

2026-08-13，DeepSeek 开源了 [DeepSeek Harness](/docs/open-source-agents/deepseek-harness)（`dsh`）。它把模型、工具、技能、session、sandbox、loop、调度和 UI 都做成可替换插件，口号是 Everything is a Plugin。这和上面的 Session / Harness / Sandbox 三分法直接对应：你可以对照阅读，看同一套构件如何被拆成 Cordis 插件，而不是只停留在 Claude Code 的产品实现里。来源：[DeepSeek Harness README](https://github.com/deepseek-ai/deepseek-harness)；[官方预览页](https://www.deepseek.com/harness/en/)。

当前仍是 developer preview，官方明确会有破坏性变更。适合学架构，不适合当作稳定生产运行时。

## 开源对照：Pi

[Pi](/docs/open-source-agents/pi) 是更小的对照样本。它把 harness 收成可读的 TypeScript 包（`pi-ai`、`pi-agent-core`、`pi-coding-agent`），默认工具只有 `read` / `write` / `edit` / `bash`，Plan、子 Agent、MCP 和权限弹窗都留给扩展。JSONL session 可分支、可 compaction，SDK 模式被 [OpenClaw](/docs/open-source-agents/openclaw) 直接嵌入。社区 Go 移植见 [pi-go](/docs/open-source-agents/pi-go)，不是官方上游。来源：[pi.dev](https://pi.dev)；[earendil-works/pi](https://github.com/earendil-works/pi)。
...
- [DeepSeek Harness](/docs/open-source-agents/deepseek-harness)：开源插件化 harness 对照。
- [Pi](/docs/open-source-agents/pi)：最小终端 harness 对照。
- [pi-go](/docs/open-source-agents/pi-go)：Pi 的社区 Go 移植，不是官方上游。
