...
Agent 一旦能发邮件、推代码、发包或调用生产 API，事故就不再只是对话失败。2026-09-15，OpenAI 公开确认正在与 Anthropic、Google 讨论安全协作，背景包括 Hassabis 7 月提出的行业 Standards Body，以及 Amodei 9 月 12 日呼吁放缓前沿能力节奏的文章。对工程团队的含义不是等待实验室联盟，而是：外部副作用工具必须有审批、审计和回滚；依赖包、技能市场、MCP server 都属于供应链，不能默认信任社区插件。来源：[CNBC，2026-09-15](https://www.cnbc.com/2026/09/15/open-ai-google-anthropic-safety.html)。

写入治理清单时至少补三问：

1. 这个 Agent 能否向包仓库、代码托管或消息渠道产生不可逆写入。
2. 技能、MCP、第三方工具的来源、权限和更新通道是否可审计。
3. 出事之后能否在分钟级冻结工具，而不是只关掉聊天窗口。
...
- [CNBC: OpenAI, Google, Anthropic discussing collaboration on AI safety](https://www.cnbc.com/2026/09/15/open-ai-google-anthropic-safety.html)：2026-09-15 实验室安全协调报道。
