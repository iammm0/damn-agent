...
| 优雅终止 | Agent 完成当前步骤后停止，不中断中间操作 | CrewAI（Callback 机制） |
| 强制终止 | 直接终止 Agent 进程或协程 | 适用于沙箱隔离的 Agent |

LangGraph 的检查点机制支持取消和恢复：每个执行步骤自动保存状态快照，取消后可以从最近的检查点恢复，而非从头开始。来源：LangGraph Persistence, https://langchain-ai.github.io/langgraph/concepts/persistence/。

CrewAI 通过回调机制支持任务取消：当某个 Agent 的任务被取消时，可以通过 `on_task_error` 回调执行清理逻辑。来源：CrewAI Documentation, https://docs.crewai.com/。
...
OpenAI Agents SDK 通过 `max_turns` 参数限制 Agent 的最大执行轮次，防止无限循环。来源：OpenAI Agents SDK Documentation, https://openai.github.io/openai-agents-python/agents/。

AutoGen 支持 `max_consecutive_auto_reply` 限制 Agent 间的最大连续回复次数，防止对话无限循环。来源：Wu et al., "AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent Conversation", arXiv:2308.08155, 2023。
...
MetaGPT 引入 SOP（标准操作流程）作为多 Agent 协作的核心约束。每个角色的输出必须满足下游角色的输入格式：ProductManager 输出 PRD → Architect 输出系统设计 → Engineer 输出代码 → QA 输出测试。这种结构化通信比自由对话更可靠。来源：Hong et al., "MetaGPT: Meta Programming for A Multi-Agent Collaborative Framework", arXiv:2308.00352, 2023。GitHub: https://github.com/geekan/MetaGPT。

MetaGPT 的关键创新是"共享消息池"——Agent 向公共消息流发布消息，而不是直接对话。这减少了信息冗余，同时保持了可追溯性。
...
CrewAI 把多 Agent 协作建模为"团队"：每个 Agent 有角色（Role）、目标（Goal）和背景故事（Backstory），任务（Task）分配给特定 Agent，Crew 定义执行流程。支持三种流程类型：Sequential（顺序执行）、Hierarchical（Manager Agent 分配任务）、Consensus（多 Agent 投票决策）。来源：CrewAI Documentation, https://docs.crewai.com/。GitHub: https://github.com/crewAIInc/crewAI。
...
| 项目 | GitHub | 核心模式 | 特点 |
| --- | --- | --- | --- |
| MetaGPT | https://github.com/geekan/MetaGPT | SOP + 角色分工 | 模拟软件公司流程 |
